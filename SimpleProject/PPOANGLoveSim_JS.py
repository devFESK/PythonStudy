...
def Chapter02():
    ...
    global health
    days = "2일차"
    txt02_1 = [f"{tem + 1.0}도의 폭염을 이겨내고, 워터파크에 도착한 우리들.", f"{name}(은/는) 수영복으로 갈아입고 깨끗하게 샤워를 했다.", "(오늘을 위해 다이어트를 했지. 귀엽고 섹시한 내 모습을 뽐내볼까..!)", f"파도 풀로 힘찬 걸음을 옮기는 {name}.", "그런데..!", "[으앗..!] / [꺅...!!]", "(강한 통증과 함께 어렴풋이 부드러운 살결이 느껴졌다.)", "(크아아악 아파라..) [저기... 괜찮으세요?]", "[지금 눈을 어디다 두고 다니시는 거예욧!!!]", "[???]", "[????????]", "(분명 나는 잘 걷고 있었는데... 내가 잘못했다는 건가? 적반하장도 가분수지.)"]
    ch.countDays(days)
    for i in txt02_1:
        for j in i:
            print(j, end = "")
